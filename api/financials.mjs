    ...
    try {
        ...
        if (cashFlow.annualReports && income.annualReports) {
            ...
            for (let i = 0; i < years; i++) {
                ...
                const capex = parseFloat(cfReport.capitalExpenditures) || 0; // Capex is usually negative in AV
                // FCF = Ops Cash - Capex. If Capex is negative, we add it? 
                // Alpha Vantage Capex is typically negative (e.g. -500). 
                // Standard FCF formula: Cash from Ops - Capex (where Capex is a positive outflow).
                // If AV gives -500, we should do: Ops + Capex (e.g. 1000 + (-500) = 500).
                // Let's check if it's negative. Yes, usually "payments for..." are negative.

                const fcf = opsCash + capex; // Adding negative capex
