...
export default async function handler(request, response) {
    ...
    try {
        ...
        // If it's a function (likely a class constructor), try to instantiate it
        if (typeof yahooFinance === 'function') {
            try {
                yahooFinance = new yahooFinance();
            } catch (e) {
                // If 'new' fails, maybe it's a factory function?
                try {
                    yahooFinance = yahooFinance();
                } catch (e2) {
                    // Ignore, we'll check for quoteSummary next
                }
            }
        }
        ...
        if (typeof yahooFinance.quoteSummary !== 'function') {
            ...
            // Also check prototype if it exists
            const protoKeys = yahooFinance.constructor && yahooFinance.constructor.prototype
                ? Object.getOwnPropertyNames(yahooFinance.constructor.prototype).join(', ')
                : 'N/A';

            throw new Error(`Import failed. Instance keys: [${keys}] | Proto keys: [${protoKeys}] | Type: ${typeof yahooFinance}`);
        }
